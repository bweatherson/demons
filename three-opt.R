...
all_cities <- uscities |> 
  ...
  #  filter(state_id %in% short_states) |>
  group_by(state_id) |>
  slice(1:40) |> 
...
city_count <- length(tour_line)

tmp <- combinations(city_count - 1, 3, layout = "row") |>
  as_tibble(.name_repair = "universal") |>
  rename(i = `...1`, j = `...2`, k = `...3`) |>
  filter(i < j - 1) |>
  filter(j < k - 1) |>
  mutate(tour_length = NA)


while(T){
start_dist <- tour_length(tour_line, tsp = as.TSP(city_lengths))

for (n in 1:nrow(tmp)){
  new_tour <- c(
    tour_line[1:tmp$i[n]],
    tour_line[tmp$k[n]:(tmp$j[n]+1)],
    tour_line[(tmp$i[n]+1):tmp$j[n]],
    tour_line[(tmp$k[n]+1):city_count]
  )
  new_length <-  tour_length(new_tour, tsp = as.TSP(city_lengths))
  tmp$tour_length[n] <- new_length
}

the_best <- tmp |>
  slice_min(order_by = tour_length, n = 1, with_ties = FALSE)

tour_line <- c(
  tour_line[1:the_best$i[1]],
  tour_line[the_best$k[1]:(the_best$j[1]+1)],
  tour_line[(the_best$i[1]+1):the_best$j[1]],
  tour_line[(the_best$k[1]+1):city_count]
)
tour_line <- solve_TSP(as.TSP(city_lengths), method = "two_opt", tour = tour_line)
if (tour_length(tour_line, tsp = as.TSP(city_lengths)) >= start_dist) break()
}
...
paths <- paths |>
  add_row(step = nrow(used_cities) + 1,
          property = "from",
          rowid = paths$rowid[1],
          lng = paths$lng[1],
          lat = paths$lat[1])
