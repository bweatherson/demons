...
Proceduralism has a pair of input conditions, and a pair of output conditions

The input conditions are:
- What is to be chosen is a function of what things are like at the start of deliberation
- The values that are relevant are knowable to the chooser as long as the chooser is rational

The output conditions are:
- Decisiveness (if multiple acceptable, then adding a penny breaks tie)
- No dilemmas

I'm going to reject every part of proceduralism.

Note that proceduralism is even more general than what @Elga2021 calls a 'suppositional' decision theory'.

Proceduralism is not a package deal - Stalnaker accepts part of it and rejects other parts - but it is tempting to see the whole thing running together.
...
The demon is really good at their job. They are not a time traveller; they are making a prediction that is not causally influenced by what the Chooser actually does. But they are really good. I'll assume that they are arbitrarily good, and come back to just what I mean by that in section \@ref(aboutademon).
...
With this in mind, we can redraw the game tree with both player's payouts listed.

Insert drawing of tree with both payouts
...
This one is actually harder for the proceduralist to answer, but it's a challenge for us too.

## Why Do Decision Theory?

The proceduralist has an answer - to advise about decision problems. What's our answer?


## Decision Theory and Making Decisions

Roughly, proceduralism must be right because decision theory is a theory of how to make decisions.

No, because of nuclear war cases
...
The recent PQ article as an example of the same kind of thing

## Dilemmas in Simple Games

Death in Damascus, including asymmetric
Versions of the ABC game

Intuitively these just are dilemmas
Note that Nash doesn't match up with intuition here

## Mixed Strategies to the Rescue?

Why we might think mixed strategies will help here

Problem one - maybe that's unintuitive
 - I'm not moved; I think rational players can do ok at rock paper scissors

Problem two - the [0, 1) choice

Problem three - the Econometrica 1995 paper example, which I'll have to find again

Feels like this won't really get us out of the jam even if you think, as I do, that others have been too quick to say chooser is rational but can't mix
