...
save(first_city, file = "/Users/weath/Dropbox (Personal)/GoodReader Transfers/first_city.RData")

first_city_full <- first_city |>
  rename(farthest_length = length) |>
  add_column(nearest_length = 0) |>
  add_column(cheapest_length = 0) |>
  add_column(easy_nearest_length = 0) |>
  add_column(easy_cheapest_length = 0) |>
  add_column(easy_farthest_length = 0)


for (i in 1:1912){
  tour_line <- solve_TSP(as.TSP(city_lengths), method = "farthest_insertion", start=i)
  first_city_full$easy_farthest_length[i] <- tour_length(tour_line)
  tour_line <- solve_TSP(as.TSP(city_lengths), method = "two_opt", tour = tour_line)
  first_city_full$farthest_length[i] <- tour_length(tour_line)
  tour_line <- solve_TSP(as.TSP(city_lengths), method = "nearest_insertion", start=i)
  first_city_full$easy_nearest_length[i] <- tour_length(tour_line)
  tour_line <- solve_TSP(as.TSP(city_lengths), method = "two_opt", tour = tour_line)
  first_city_full$nearest_length[i] <- tour_length(tour_line)
  tour_line <- solve_TSP(as.TSP(city_lengths), method = "cheapest_insertion", start=i)
  first_city_full$easy_cheapest_length[i] <- tour_length(tour_line)
  tour_line <- solve_TSP(as.TSP(city_lengths), method = "two_opt", tour = tour_line)
  first_city_full$cheapest_length[i] <- tour_length(tour_line)
}

save(first_city_full, file = "/Users/weath/Dropbox (Personal)/GoodReader Transfers/first_city_full.RData")
